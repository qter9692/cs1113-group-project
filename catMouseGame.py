...
# Note that pos is accessed as state.cat["xcord"], and delta-pos
...
def updateState(state):
    ...
    if ((state.mouse["xcord"]-50) < state.cheese["xcord"]  < (state.mouse["xcord"]+50) and (state.mouse["ycord"]-50) < state.cheese["ycord"] < (state.mouse["ycord"]+50)):
        ...
        return (state)
                ],  state.cat["xvelocity"] ,state.cat["ycord"] + state.cat["yvelocity"], state.cat["yvelocity"] , state.mouse["xcord"] + state.mouse["xvelocity"], state.mouse["xvelocity"], state.mouse["ycord"] + state.mouse["yvelocity"], state.mouse["yvelocity"] ,(randint(0,420)),randint(0,420))
    else:
...
def endState(state):
    if ((state.cat["xcord"]-50) < state.mouse["xcord"] and state.mouse["xcord"] < (state.cat["xcord"]+80) and (state.cat["ycord"]-50) <state.mouse["ycord"] and state.mouse["ycord"] < (state.cat["ycord"]+80)):
        return True
    else:
        return False 
...
class State:
    cat = {"xcord": randint(250,350),
               "ycord": randint(250,350),
               "xvelocity": x,
               "yvelocity": y
               }
    mouse = {"xcord": 50,
                 "ycord": 50,
                 "xvelocity": randint (-1,1),
                 "yvelocity": randint (-1,1)
                 }
    cheese = {"xcord": randint (0,450),
                  "ycord": randint (0,450)
                  }   
        
ooInitState = State()
ooInitState.cat["xcord"]
